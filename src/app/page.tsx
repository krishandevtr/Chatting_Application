...
async function getUsers(): Promise<User[]> {
	...
	const results = (await pipeline.exec()) as (User | null)[];
	
	if (!results || results.length === 0) {
		// If the pipeline execution returned no results
		return [];
	}

	const users: User[] = results.filter((user) => user && user.id !== currentUser?.id) as User[];
	console.log(users,"Users that fetched")
	return users;
}
